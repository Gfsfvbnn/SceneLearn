...
export default function Home() {
  ...
  const handleSaveWord = async (item: VocabItem) => {
    ...
    if (!user) {
      setError("Ban can dang nhap de luu tu vung");
      return;
    }

    const { error: insertError } = await supabase.from("looked_up_words").insert({
      user_id: user.id,
      word: item.word,
    ...
    if (!insertError) {
      setSavedWords((prev) => [...prev, item.word]);
    } else {
      console.error("Loi khi luu tu vung:", insertError.message);
    }
  };
    ...
    <div className="min-h-screen dot-grid flex flex-col items-center py-16 px-4">
      <div className="decorative-bg w-full max-w-2xl">
        <div className="relative z-10">
          ...
          {vocabulary.length > 0 && (
            <div className="mt-8">
              <h2 className="text-xl font-semibold mb-3" style={{ color: "#10233F" }}>
                Tu vung
              </h2>
